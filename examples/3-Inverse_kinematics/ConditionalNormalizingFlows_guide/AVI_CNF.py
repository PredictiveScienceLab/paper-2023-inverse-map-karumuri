...
# Training
optim = torch.optim.Adam(params=[p for p in m.model.parameters() if p.requires_grad==True], lr=lr, amsgrad=True)
...
for step in range(n_steps):
    ...
    if step % 1000 == 0: 
        print('step %s:' % step, 'loss = %.3f,' % loss)
    ...
    if (step % 10000 == 0) or (step == n_steps - 1):
        ...
        }, os.path.join(results_dir,'checkpoint_{}.pt'.format(step)))

cnf_model_state_dict = m.state_dict()
torch.save(cnf_model_state_dict, os.path.join(results_dir,'cnf_model_state_dict.pt'))

np.save(os.path.join(results_dir,'steps.npy'), np.asarray(steps))
np.save(os.path.join(results_dir,'losses.npy'), np.asarray(losses))
np.save(os.path.join(results_dir,'learning_rates.npy'), np.asarray(learning_rates))
...
def plot(xi_samples, xi_data, y_data, j, title, color_code, show=False, save=True):
    ...
    if save is True:
        plt.savefig(os.path.join(results_dir,str(j)+'_plot'+'.pdf'), dpi=300)
        torch.save(xi_samples, os.path.join(results_dir,str(j)+'_xi_samples.pt'))
        #xi_samples = torch.load(os.path.join(results_dir,str(j)+'_xi_samples.pt'))
    if show is True:
        plt.show()
    plt.close()
...
def pair_plot(df, xi_data, j, show=False, save=True):
    ...
    if save is True:
        plt.savefig(os.path.join(results_dir,str(j)+'_pairplot'+'.pdf'), dpi=300)
    if show is True:
        plt.show()
...
# # Getting data
# xi_data_, y_data_ = torch.load(os.path.join(results_dir,'xi_data.pt')), torch.load(os.path.join(results_dir,'y_data.pt'))
# print(xi_data_.shape, y_data_.shape)

save = True
if save is True:
    torch.save(xi_data_, os.path.join(results_dir,'xi_data.pt'))
    torch.save(y_data_, os.path.join(results_dir,'y_data.pt'))
...
for j in range(y_data_.shape[0]): 
    ...
    if prior:
        ...
        color_code = colorlist[j] #random.randint(0, 3)
    ...
    # xi_samples_MCMC = torch.load(os.path.join(results_dir, str(j)+'_MCMC'+'_xi_samples.pt'))
    # print(xi_samples_MCMC.shape)
